                ...
                <div className="cap-grid">
                  {capResult.opciones.map((op) => (
                    <div key={op.sistema} className={`cap-card${op.viable ? " is-viable" : " is-no"}`}>
                      ...
                      {/* ── Diseñar layout (solo nidal) ── */}
                      {op.viable && op.sistema === "nidal_colectivo" && (
                        <div className="layout-section">
                          <div className="layout-divider" />
                          <div className="layout-title">Diseñar layout en planta</div>
                          {!layoutResult && !layoutLoading && (
                          ...
                          {layoutLoading && (
                            <div className="layout-loading">
                              <div className="loading-dots loading-dots--sm"><span /><span /><span /></div>
                              <span>Gemini diseñando el layout…</span>
                            </div>
                          )}

                          {layoutResult && !layoutLoading && (
                            <div className="layout-result">
                              <button className="layout-reset" onClick={() => { setLayoutResult(null); setLayoutExterior(""); }}>
                                ← Nuevo layout
                              </button>

                              {layoutResult.error && (
                                <div className="layout-error">{layoutResult.error}</div>
                              )}

                              {layoutResult.viable && (
                                <>
                                  <div className="layout-filas">
                                    {layoutResult.filas.map((fila, i) => (
                                      <div key={i} className="layout-fila-row">
                                        <span className="layout-fila-num">Fila {i + 1}</span>
                                        <span className="layout-fila-detail">
                                          {fila.num_modulos} módulos
                                          {fila.pegada_pared_izq ? " · pegada pared izq" : ""}
                                          {fila.pegada_pared_der ? " · pegada pared der" : ""}
                                        </span>
                                        <span className="layout-fila-slot">
                                          slot {fila.slot_izq}m + {fila.slot_der}m
                                        </span>
                                      </div>
                                    ))}
                                  </div>
                                  <div className="layout-stats">
                                    <div className="layout-stat">
                                      <span className="layout-stat-label">Total módulos</span>
                                      <span className="layout-stat-val">{layoutResult.total_modulos}</span>
                                    </div>
                                    <div className="layout-stat">
                                      <span className="layout-stat-label">Max. gallinas</span>
                                      <span className="layout-stat-val">{layoutResult.max_gallinas.toLocaleString("es-ES")}</span>
                                    </div>
                                    <div className="layout-stat">
                                      <span className="layout-stat-label">Yacija interior</span>
                                      <span className={`layout-stat-val${!layoutResult.cumple_normativa ? " is-warn" : ""}`}>
                                        {layoutResult.yacija_interior_m2.toFixed(1)} m²
                                      </span>
                                    </div>
                                    <div className={`layout-stat layout-stat--norm${layoutResult.cumple_normativa ? " is-ok" : " is-fail"}`}>
                                      <span className="layout-stat-label">Normativa</span>
                                      <span className="layout-stat-val">{layoutResult.cumple_normativa ? "✓ Cumple" : "✗ No cumple"}</span>
                                    </div>
                                  </div>
                                  <p className="layout-explicacion">{layoutResult.explicacion}</p>
                                </>
                              )}

                              {/* Escalada a exterior si yacija insuficiente */}
                              {layoutResult.necesita_exterior && (
                                <div className="layout-exterior-ask">
                                  <p className="layout-exterior-msg">
                                    La yacija interior no es suficiente ({layoutResult.deficit_yacija_m2.toFixed(1)} m² de déficit).
                                    ¿Dispones de zona exterior para compensar?
                                  </p>
                                  <form className="layout-form" onSubmit={(e) => {
                                    e.preventDefault();
                                    onLayoutSubmit(parseFloat(layoutExterior));
                                  }}>
                                    <div className="field-input-wrap">
                                      <input
                                        type="number" min={0} step="0.1" required placeholder="Superficie exterior"
                                        className="field-input field-input--unit"
                                        value={layoutExterior}
                                        onChange={(e) => setLayoutExterior(e.target.value)}
                                      />
                                      <span className="field-unit">m²</span>
                                    </div>
                                    <button type="submit" className="btn-pill btn-pill--sm" disabled={!layoutExterior}>
                                      Recalcular con exterior
                                      <svg width="12" height="8" viewBox="0 0 12 8" fill="none"><path d="M1 4h10M7 1l4 3-4 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                                    </button>
                                  </form>
                                </div>
